...
export const SchedulePage = () => {
  ...
  const fetchEvents = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:8000/v1/operations/calendar/events?farm_id=${farmId}`,
        {
          method: "GET",
          headers: {
  ...
  const createNewEvent = async (eventData) => {
    try {
      ...
      const response = await fetch(
        "http://localhost:8000/v1/operations/events",
        {
          method: "POST",
          headers: {
            accept: "application/json",
  ...
  const NewEventModal = () => {
    ...
    const handleSubmit = (e) => {
      ...
      // If repeating, create additional events
      if (repeatFrequency !== "Does not repeat") {
    ...
    const createRepeatingEvents = () => {
      ...
      // Skip the first event since it's already created by createNewEvent
      // and move to the next occurrence
      switch (repeatFrequency) {
        case "Daily":
          currentStart.setDate(currentStart.getDate() + repeatInterval);
          currentEnd.setDate(currentEnd.getDate() + repeatInterval);
          break;
        case "Weekly":
          currentStart.setDate(currentStart.getDate() + 7 * repeatInterval);
          currentEnd.setDate(currentEnd.getDate() + 7 * repeatInterval);
          break;
        case "Monthly":
          currentStart.setMonth(currentStart.getMonth() + repeatInterval);
          currentEnd.setMonth(currentEnd.getMonth() + repeatInterval);
          break;
        default:
          break;
      }
      ...
      while (currentStart <= untilDate) {
        const eventData = {
          ...
          endDate: moment(currentEnd).format("YYYY-MM-DD"),
        };

        // Create the repeating event locally
        createEventLocally(eventData);
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-lg p-4 sm:p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {/* Repeat Field - Added below Assigned To */}
            <div>
            ...
            {/* Repeat Options - Only show when repeating is selected */}
              ...
              <div className="flex flex-wrap gap-2">
                {colorOptions.map((color, idx) => (
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      setNewTask((prev) => ({ ...prev, taskColor: color }))
                    }
                    className={`w-8 h-8 rounded-full border-2 ${
                      newTask.taskColor === color
                        ? "border-gray-900"
                        : "border-gray-300"
                    }`}
                    style={{ backgroundColor: color }}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setNewTimesheet((prev) => ({ ...prev, date: e.target.value }))
                }
                className="w-full px-3 py-2 border rounded-md text-sm"
                required
              />
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setNewTimesheet((prev) => ({ ...prev, task: e.target.value }))
                }
                className="w-full px-3 py-2 border rounded-md text-sm"
                required
              />
                      ...
                      {timesheets.map((timesheet) => (
                        <tr key={timesheet.id} className="border-t">
                          <td className="px-3 py-2 text-xs">
                            {moment(timesheet.date).format("MMM D")}
                          </td>
                          <td className="px-3 py-2 text-xs">
                            {timesheet.task}
                          </td>
